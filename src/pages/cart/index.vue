  ...
  <div class="page">
    ...
    <div class="goodsList">
      ...
      <div class="item" v-for="(value, key) in goodsList" :key="key">
        <div class="check">
          <icon type="success" @click="select(value)" size="20" :color="value.select ? 'red': 'gray'"></icon>
        </div>
        <div class="goods">
          <div class="left">
  ...
  methods: {
    ...
    selectAll() {
      ...
      Object.keys(this.goodsList).forEach(v => {
        this.goodsList[v].select = select
      })
    },
    // 点击结算按钮，将商品数据提交到服务器中
    async jiesuan() {
      ...
      Object.keys(this.goodsList).forEach(v => {
        ...
        if (obj.select) {
          ...
          orderItem.selectStatus = obj.select
